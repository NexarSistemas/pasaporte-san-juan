-- Sincroniza el historial local con los campos editoriales ya presentes en
-- producción. Las preguntas existentes del seed quedan publicadas para que
-- una instalación nueva conserve el comportamiento actual del juego.
alter table public.preguntas
  add column texto_original text,
  add column estado_editorial text not null default 'pendiente'
    check (estado_editorial in ('pendiente', 'en_revision', 'revisada', 'publicada', 'rechazada')),
  add column observaciones_revision text,
...
create index preguntas_estado_editorial_idx
  on public.preguntas (estado_editorial);

create index preguntas_categoria_estado_editorial_idx
  on public.preguntas (categoria_id, estado_editorial);
...
begin
  ...
  where activo and estado_editorial = 'publicada';
  if v_activas = 0 then
