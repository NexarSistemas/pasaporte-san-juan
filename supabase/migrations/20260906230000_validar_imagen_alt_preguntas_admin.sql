-- Exige texto alternativo para imágenes al actualizar preguntas desde administración.
...
begin
  ...
  if nullif(pg_catalog.btrim(coalesce(p_imagen, '')), '') is not null
    and nullif(pg_catalog.btrim(coalesce(p_imagen_alt, '')), '') is null then
    raise exception 'La imagen requiere un texto alternativo.' using errcode = '22023';
  end if;
